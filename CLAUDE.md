...
- **Entailment checking (`entailment.py`) is off by default** (`DPDP_ENTAILMENT=1`, needs `cross-encoder/nli-deberta-v3-base`). Checks whether the cited provisions actually *support* each sentence of the answer — the gap `check_citations()` cannot cover, and the direct fix for the §14 "family vs nominated person" bug. Results appear as a `claims` field on the `citations` SSE event and in the audit log. **Code-complete but not yet validated** — the model download filled the disk and was removed; measure it against `answer_eval.yaml`'s §14 case first. Note: use `huggingface_hub` with explicit `allow_patterns` when fetching it — an unfiltered `snapshot_download` pulls TF/ONNX/OpenVINO variants (~3.6GB) rather than the ~400MB actually needed.
